...
async def _execute_task(
    ...
    # 1. Outputs from prerequisite tasks
    dependency_context = "\n\n".join(
        f"OUTPUT FROM {dependency}:\n{outputs[dependency]}"
        for dependency in task.depends_on
    ) or "No prerequisite outputs."

    # 2. Project pipeline context
    pipeline_context = ""

    if pipeline is not None:

        recalled = pipeline.recall(task.instruction)

        kb = None
        if pipeline.needs_knowledge(task.instruction):
            kb = await pipeline.retrieve(task.instruction)

        context_blocks = pipeline.compose_context(
            task.instruction,
            recalled,
            kb,
        )
...
Project context:
{pipeline_context or "No additional project context available."}
...
"""
    ...
    # LLM invoke is synchronous, so move it off the async event loop.
...
async def execute_plan(
    ...
    for batch in plan.execution_batches():
        ...
        # Tasks in the same batch are independent,
